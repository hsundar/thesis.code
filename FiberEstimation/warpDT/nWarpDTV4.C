...
{
   cout << "\nUsage:\n";
   cout << "   " << runname <<" <inputDTVolume> <warpVF> <PD> -P<2nd_PDfile> -O<outfile> -Z<slice> -R<rx,ry,rz> -f -N<0/1>" <<endl;
   cout << "   " << runname <<" -H" <<endl;
   ...
   cout << "  -X<dim>        : X or Y dimensions of the warped volume, default: 256 \n";								      
...
{   
   ...
   if (argc==1){
     ...
     flipZ=0;
     while ((c=getopt(argc-3,argv+3,"HP:O:Z:C:fR:N:X:")) != -1) {
...
{//warp DT, this is the real Main()
   ...
   // needs to be removed --- PK's Q to XDR :what and why ?
   ...
   ***** for debugging purpose ****/
   ...
   mSVD->loadIdentity(); // to disable re-orientation
   //DTVolume *volDT;
   //getchar(); 
   //volDT->reorientTensor(mSVD);// commented to disable re-orientation
   delete mSVD;
