...
public: 
  ...
  **/
  ...
  Point GetCenterInMM() {
    return Pixel2MM(GetCenterInPixel());
  };


  int GetVolBitsAllocated() const {
    return m_VolBitsAllocated;
  };


  int GetVolBitsStored() const {
    return m_VolBitsStored;
  };
  ...
  **/
  ...
  double GetVoxelSize( int dim ) const {
    return m_VoxelSize[dim];
  };
  ...
  **/
  ...
  void SetVoxelSize( int dim, double voxelSize ) {
    m_VoxelSize[dim] =       
    voxelSize;
  };
  ...
  **/
  ...
  double GetCorner000( int dim ) const {
    return m_Corner000[dim];
  };

  Point GetCorner000() const {
    return												
  ...
  **/
  ...
  void SetCorner000( int dim, double corner ) {
    m_Corner000[dim] = corner;
  };

  void SetCorner000( double* corner ) {
    m_Corner000[0] =              
    corner[0];m_Corner000[1] =
    corner[1];m_Corner000[2] = corner[2];
  };

  void SetCorner000( Point inPoint ) {
    m_Corner000[0] =             
  ...
  **/
  ...
  void SetOrientationX( double* orientation ) {
    m_OrientationX[0] =       
  ...
  void SetOrientationY( double* orientation ) {
    m_OrientationY[0] =       
  ...
  void SetOrientationZ( double* orientation ) {
    m_OrientationZ[0] =       
  ...
  void SetOrientationX( Point orientation ) {
    m_OrientationX[0] =         
  ...
  void SetOrientationY( Point orientation ) {
    m_OrientationY[0] =         
  ...
  void SetOrientationZ( Point orientation ) {
    m_OrientationZ[0] =         
  ...
  double GetOrientationX( int dim ) const {
    return m_OrientationX[dim];
  };

  double GetOrientationY( int dim ) const {
    return m_OrientationY[dim];
  };

  double GetOrientationZ( int dim ) const {
    return m_OrientationZ[dim];
  };


  Point GetOrientationX() const {
    return											

    Point(m_OrientationX[0],m_OrientationX[1],m_OrientationX[2]);
  };

  Point GetOrientationY() const {
    return											

    Point(m_OrientationY[0],m_OrientationY[1],m_OrientationY[2]);
  };

  Point GetOrientationZ() const {
    return											
  ...
  **/
  ...
  std::vector<Node>& getOctree() {
    return m_Octree;
  };

  unsigned int octSize() {
    return m_Octree.size();
  }
  Node& getNode(unsigned int i) {
    return m_Octree[i];
  }
  Point getNodeCenter(unsigned int i) {
    return m_Octree[i].center() ;
  }
  ...
  unsigned char * getLowRes() {
    return m_ucpLowRes;
  }

  unsigned char * getHighRes() {
    return m_ucpHighRes;
  }

  int GetNumberOfLandmarks() {
    return m_iNumberOfLandmarks;
  }
  ...
  // fast inline access to low and high-res versions ...
  unsigned char GetLowRes(Point pt) {
    int x = pt.xint();
    int y = pt.yint();
    int z = pt.zint();

    int xmax = ((m_Dims[0] >> 2) << 2);
    int ymax = ((m_Dims[1] >> 2) << 2);
    int zmax = ((m_Dims[2] >> 2) << 2);
    ...
    return  m_ucpLowRes[(m_Dims[0]>>2)*(z*(m_Dims[1]>>2) + y) + x];
  }

  unsigned char GetHighRes(Point pt) {
    int x = pt.xint();
    int y = pt.yint();
    int z = pt.zint();
    ...
    int index = (m_Dims[0]>>1)*(z*(m_Dims[1]>>1) + y) + x;

    return m_ucpHighRes[index];
  }
...
protected:
  ...
  // Data for fast rigid registration.
  // generate subsampled data that is binned to 6 bits.
  unsigned char *m_ucpLowRes;
  unsigned char *m_ucpHighRes;
};
