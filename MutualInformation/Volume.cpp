...
bool Volume::InitMhd(QString filename)
{
  ...
  */
  ...
  while (!feof(fp_dat)) 
  {
    ...
    if (strncmp(temp, "Offset =", strlen("Offset =")) == 0) 
    {
      sscanf(temp, "%s %s %lf %lf %lf", tmp2, c, &m_Corner000[0],
          &m_Corner000[1], &m_Corner000[2]);
    }
...
bool Volume::InitAnalyze(QString filename)
{
  ...
  // orientation ..
  m_OrientationX[0] = 1.0; m_OrientationX[1] = 0.0; m_OrientationX[2] = 0.0;
  m_OrientationY[0] = 0.0; m_OrientationY[1] = 1.0; m_OrientationY[2] = 0.0;
  m_OrientationZ[0] = 0.0; m_OrientationZ[1] = 0.0; m_OrientationZ[2] = 1.0;
...
void Volume::ComputeLinearOctree() {
  ...
  // std::cout << "Octsize is " << octSz << std::endl;
...
bool Volume::isUnique(Point _off, Point _sz) {
  ...
  unsigned char val = *((unsigned char *)GetAt(mid.x(), mid.y(), mid.z()));
  ...
  for (int k=floor(lower.z()); k<ceil(upper.z()); k++)
    for (int j=floor(lower.y()); j<ceil(upper.y()); j++)
      for (int i=floor(lower.x()); i<ceil(upper.x()); i++) {
