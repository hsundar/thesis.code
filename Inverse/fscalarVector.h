 ...
 *  derived class.
 */
#ifndef _FSCALARVECTOR_H_
#define _FSCALARVECTOR_H_
...
class fscalarVector : public feVector<fscalarVector>
{
  public:	
    fscalarVector(daType da);
    /**
...
fscalarVector::fscalarVector(daType da) {
#ifdef __DEBUG__
...
bool fscalarVector::initStencils() {
  typedef int* int1Ptr;
  typedef int** int2Ptr;
...
bool fscalarVector::ElementalAddVec(unsigned int i, PetscScalar *in, double scale){
  ...
  for (int k = 0;k < 8;k++) {
	 //    for (int j=0;j<8;j++) {
	 //      in[m_uiDof*idx[k]] += fac*(Aijk[elemType][k][j])*fscalar;
...
bool fscalarVector::ElementalAddVec(int i, int j, int k, PetscScalar ***in, double scale){
  int dof=m_uiDof;
  int idx[8][3]={
    {k, j, dof*i},
    {k,j,dof*(i+1)},
...
bool fscalarVector::postAddVec()
{
  return true;
}


#endif /*_FSCALARVECTOR_H_*/
